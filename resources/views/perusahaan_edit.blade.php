...
<head>
    ...
    <title>Edit Perusahaan | Karir Tako</title>
...
<body class="bg-BgTako text-DarkTako h-full">
    <div class="py-16 px-16">
        <div class="p-8 container mx-auto bg-white h-full rounded-2xl">
            ...
            <form id="vue" action="{{ url('/admin/perusahaan/update_perusahaan') }}" method="POST" class="w-full"
                enctype="multipart/form-data">
                @csrf
                <input class="" type="text" name="id" id="id" value="{{ $perusahaan[0]->id }}"
                    hidden>
                <div class="flex flex-wrap pt-8 gap-4">
                    <label for="perusahaan" class="w-[49%]">
                        <h1 class="mb-2">Nama Perusahaan</h1>
                        <input type="text" name="perusahaan" value="{{ $perusahaan[0]->perusahaan }}" required
                            class="rounded-2xl w-full" placeholder="Masukkan Nama Perusahaan">
                    </label>
                    <label for="link" class="w-[49%]">
                        <h1 class="mb-2">Link Perusahaan (jika ada)</h1>
                        <input type="text" name="link" value="{{ $perusahaan[0]->link }}" required
                            class="rounded-2xl w-full" placeholder="Masukkan Link Perusahaan">
                    </label>

                    <label for="alamat" class="w-full">
                        <h1 class="mb-2">Alamat Perusahaan</h1>
                        <input type="text" name="alamat" value="{{ $perusahaan[0]->alamat }}" required
                            class="rounded-2xl w-full" placeholder="Masukkan Alamat Perusahaan">
                    </label>
                    ...
                    <label for="image" class="w-full">
                        ...
                        <img id="image-preview" src="{{ asset('storage/images/' . $perusahaan[0]->image) }}"
                            alt="Logo Perusahaan" style="max-width: 100px" />
                        <input type="file" name="image" placeholder="Masukkan Logo Perusahaan" id="image-input">
                    </label>
